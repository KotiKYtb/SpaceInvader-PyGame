...
#On créer une variable "fps" qui contient la valeur "60" qui va être le taux de rafréchiss   ement de la fenêtre
fps = 60
...
police_écrature = pygame.font.Font("LivetInvaderIMG/DS-DIGII.TTF", 32)

#On créer la fenêtre avec pour paramètre, hauteur et largeur
fenetre = pygame.display.set_mode((largeur_fenetre, hauteur_fenetre))
#On ajoute un nom à la fenêtre
pygame.display.set_caption('Livet Invader')

#On charge l'image de fond pour la fenêtre
fond = pygame.image.load("LivetInvaderIMG/BackgroundInvader.png")
#On charge une autre image qui elle servira de GUI
borne = pygame.image.load("LivetInvaderIMG/borne.png")
joystick = pygame.image.load("LivetInvaderIMG/Joystick.png")
joystick_droit = pygame.image.load("LivetInvaderIMG/Joystick_droit.png")
joystick_gauche = pygame.image.load("LivetInvaderIMG/Joystick_gauche.png")
spacebuttonoff = pygame.image.load("LivetInvaderIMG/SpaceButtonOff.png")
spacebuttonon = pygame.image.load("LivetInvaderIMG/SpaceButtonOn.png")
life_barre = pygame.image.load("LivetInvaderIMG/life_barre.png")

#On défini quelques variables pour nos class/fonction
rangés = 4   #La variable "rangé" vaut 4
colonne = 6 #La variable "colonnes" vaut 4
...
class Vaisseau(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, life, point=0):
        pygame.sprite.Sprite.__init__(self) #La methode "__init__" est donc associer à un groupe avec sprite
        self.image = pygame.image.load("LivetInvaderIMG/elève.png")  #On charge l'image "vaisseau.png" à la fonction "image" qui sera mis sur la surface de la fenêtre
    ...
    def update(self):
        ...
        if touche[pygame.K_q] and self.rect.left > 0:   #Si la touche "a" est préssée et que la zone de collision soit supérieur à 0 en partant de la gauche
            self.rect.x -= vitesse_joueur   #On enlève la valeur de vitesse_joueur à la zone de collision en x
        if touche[pygame.K_d] and self.rect.right < largeur_fenetre:    #Si la touche "d" est préssée et que la zone de collision soit inférieur à la largeur de la fenêtre (droite)
            self.rect.x += vitesse_joueur  #On ajoute la valeur de vitesse_joueur à la zone de collision en x
        if touche[pygame.K_SPACE] and temps_laser - self.dernier_tir > cooldown:    #Si la touche "espace" est préssée et que le temps du laser - le temps du dernier tir soit supérieur au temps de rechargement
...
class Laser(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        pygame.sprite.Sprite.__init__(self) #La methode "__init__" est donc associer à un groupe avec sprite
        self.image = pygame.image.load("LivetInvaderIMG/carte_graphique.png") #On charge l'image "laser.png" à la fonction "image" qui sera mis sur la surface de la fenêtre
        self.rect = self.image.get_rect()   #On récupère avec "get.rect", la zone de collision de l'image pour l'associer à la fonction "rect" qui est donc la collision
    ...
    def update(self):
        ...
        if pygame.sprite.spritecollide(self, ennemies_group, True): #Si la zone de collision d'un groupe(sprite) ici du groupe ennemies_group. Avec le paramètre "True" et bien il faut que la condition soit absolument Vrai
            ...
            vaisseau.point_player += random.randint(25, 150)
...
class Ennemies(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        pygame.sprite.Sprite.__init__(self) #La methode "__init__" est donc associer à un groupe avec sprite
        self.image = pygame.image.load("LivetInvaderIMG/ennemies" + str(random.randint(1, 3)) + ".png") #On défini la variable "image" au chargement de l'image "ennemies.png" mais ici avec de l'aléatoire
        self.rect = self.image.get_rect()   #On récupère avec "get.rect", la zone de collision de l'image pour l'associer à la fonction "rect" qui est donc la collision
        self.rect.center = [x, y]   #On place donc la collision qui contient l'image, donc au centre de la fenêtre en x et à une valeur de y
        self.compteur_mouvement = 0 #On défini la variable compteur_mouvement à 0
        self.sens_mouvement = 1 #On défini la variable sens_mouvement à 1
...
class Ennemies_Laser(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        pygame.sprite.Sprite.__init__(self) #La methode "__init__" est donc associer à un groupe avec sprite
        self.image = pygame.image.load("LivetInvaderIMG/prof_laser.png")    #On défini la variable "image" au chargement de l'image "ennemies_laser.png"
        self.rect = self.image.get_rect()   #On récupère avec "get.rect", la zone de collision de l'image pour l'associer à la fonction "rect" qui est donc la collision
        self.rect.center = [x, y]   #On place donc la collision qui contient l'image, donc au centre de la fenêtre en x et à une valeur de y
...
créer_ennemies()    #On fais appel à la fonction dès l'exécution du programme
...
start = True    #On défini la variable start à 0
...
while start:    #Tant que start à pour valeur True et bien la boucle continue de fonctionner
    ...
    clock.tick(fps) #On défini la variable clock créer au tout début, on lui affècte la méthode tick avec la valeur de fps qui était de 60 (le taux de rafraichissement de la fenêtre)

    y_fond += 3 #On ajoute +3 à chaque tick pour y_fond
    
    joystick_touche = pygame.key.get_pressed()
    
    point = police_écrature.render(str(vaisseau.point_player), True, (255,255,255))
    ...
    if y_fond < 1080: #Si la variable y_fond est inférieur à 1080 (la taille de l'image de fond) alors:
        ...
        y_fond = 0  #On redéfini la variable y_fond à 0
        fenetre.blit(fond, (0, y_fond)) #On utilise la méthode blit qu'on associe à la variable fenêtre créer au début et ensuite lui associer la variable fond contenant l'image de fond
...
#On quitte le programme à la fin si on sort de la boucle While par erreur, sinon cette ligne n'est ici que par sécurité
pygame.quit()
